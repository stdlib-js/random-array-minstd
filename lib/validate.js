...
function validate( opts, options, mode ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1eH2V,FD', options ) );
	}
	if ( mode === 0 ) {
		if ( hasOwnProp( options, 'idtype' ) ) {
			opts.idtype = options.idtype;
			if ( RDTYPES.indexOf( opts.idtype ) < 0 ) {
				return new TypeError( format( '1eH2X,3g,4S,GD,Gg,Jm', 'idtype', RDTYPES.join( '", "' ), opts.idtype ) );
			}
		}
		if ( hasOwnProp( options, 'ndtype' ) ) {
			opts.ndtype = options.ndtype;
			if ( FDTYPES.indexOf( opts.ndtype ) < 0 ) {
				return new TypeError( format( '1eH2X,3g,4S,GD,Gg,Jm', 'ndtype', FDTYPES.join( '", "' ), opts.ndtype ) );
			}
		}
	} else if ( mode === 1 && hasOwnProp( options, 'dtype' ) ) {
		opts.dtype = options.dtype;
		if ( RDTYPES.indexOf( opts.dtype ) < 0 ) {
			return new TypeError( format( '1eH2X,3g,4S,GD,Gg,Jm', 'dtype', RDTYPES.join( '", "' ), opts.dtype ) );
		}
	} else if ( hasOwnProp( options, 'dtype' ) ) { // mode === 2
		opts.dtype = options.dtype;
		if ( FDTYPES.indexOf( opts.dtype ) < 0 ) {
			return new TypeError( format( '1eH2X,3g,4S,GD,Gg,Jm', 'dtype', FDTYPES.join( '", "' ), opts.dtype ) );
		}
	}
	return null;
}
